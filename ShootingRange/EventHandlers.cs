    ...
    public class EventHandlers
    {
        ...
        public void BringRangers()
        {
            ...
            foreach (Player plyr in rangerList)
            {
                ...
                Log.Debug($"Removing {plyr.Nickname} from the range...", PluginMain.Instance.Config.DebugMode);
            ...
            Log.Debug($"Removed {toRemoveList.Count} amount of people from the tracking list", PluginMain.Instance.Config.DebugMode);
        }
        ...
        public IEnumerator<float> WaitForRespawnCoroutine()
        {
            for (;;)
            {
                ...
                {
                    Log.Debug($"ATTEMPTING TO BRING BACK ALL {rangerList.Count} RANGERS!", PluginMain.Instance.Config.DebugMode);
                    BringRangers();
                }
        ...
        public IEnumerator<float> CheckBoundsCoroutine()
        {
            for (;;)
            {
                foreach (Player plyr in Player.List)
                {
                    ...
                    {
                        if(PluginMain.Instance.Config.RaBoundsImmunity&&plyr.RemoteAdminAccess)
                        {
                            continue;
                        }
                        plyr.ClearInventory();
        ...
        public IEnumerator<float> SpawnTargetsCoroutine()
        {
            ...
            int distance = PluginMain.Instance.Config.AbsoluteTargetDistance;
            int distanceZbetween = PluginMain.Instance.Config.RelativeTargetDistance;
            ...
            for (var i=0;i<3;i++)
            {
                ShootingTargetToy.Create(ShootingTargetType.Sport, new Vector3(startTargetsX - i * distanceXBetweeen, height, -53 - distance - distanceZbetween * i), rot.eulerAngles);
                ShootingTargetToy.Create(ShootingTargetType.ClassD, new Vector3(startTargetsX - 1 - distanceXBetweeen * 3 - distanceXBetweeen * i, height, -53 - distance - distanceZbetween * i), rot.eulerAngles);
                ShootingTargetToy.Create(ShootingTargetType.Binary, new Vector3(startTargetsX - 2 - distanceXBetweeen * 6 - distanceXBetweeen * i, height, -53 - distance - distanceZbetween * i), rot.eulerAngles);
            }
